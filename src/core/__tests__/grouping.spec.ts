...
// 对——比如 targets 传错，这套函数级测试测不出来，需要挂载组件的测试才能
// 补上。committedResources（即这里的 resources 参数）传参已被
// src/views/__tests__/DevelopmentView.spec.ts 的真实 SFC 挂载测试覆盖
// （不依赖 @vue/test-utils），targets 传参仍是盲区。
