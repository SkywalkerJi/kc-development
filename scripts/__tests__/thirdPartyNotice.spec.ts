...
describe('buildThirdPartyNotice（纯函数）', () => {
  ...
  it('commit 为 null 时（KC3 clone 不是 git 仓库）诚实标注未知，不伪造一个占位 commit', () => {
    const notice = buildThirdPartyNotice({
      source: 'https://github.com/KC3Kai/kc3-translations',
      commit: null,
      licenseText: 'MIT license text',
    })
    expect(notice).toMatch(/unknown/)
    expect(notice).not.toMatch(/Commit:\s*null/)
  })
...
describe('THIRD_PARTY_NOTICES（仓库产出物回归校验）', () => {
  ...
  it('仓库根目录存在 THIRD_PARTY_NOTICES，且记录了与 _meta.json 相同的 commit', () => {
    expect(meta.commit).toBeTruthy() // 本仓库的 vendor clone 是真实 git 仓库，commit 应能解析到
    expect(rootNotice).toContain(meta.commit as string)
  })

  it('public/data/i18n/ 下也有一份 THIRD_PARTY_NOTICES（随构建产物一起分发），记录同一个 commit', () => {
    expect(i18nNotice).toContain(meta.commit as string)
  })
